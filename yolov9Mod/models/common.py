...
class Detections:
    ...
    def to_json(self):
        ...
        # Check if xyxy is not empty and the first DataFrame is not empty
        if new.xyxy and not new.xyxy[0].empty:
            # Select the desired columns and rename them
            df = new.xyxy[0][['xmin', 'ymin', 'xmax', 'ymax', 'confidence', 'class', 'name']]
            df.columns = ['x1', 'y1', 'x2', 'y2', 'confidence', 'class', 'name']
            
            fruit_names = df['name'].tolist()
            fruit_counts = Counter(fruit_names)
            
            grouped = df.groupby('name')
            bounding_boxes = grouped.apply(lambda x: x[['x1', 'y1', 'x2', 'y2', 'confidence']].to_dict(orient='records')).tolist()
            
            # Create a new DataFrame with the desired structure
            json_df = pd.DataFrame(list(fruit_counts.items()), columns=['name', 'quantity'])
            json_df['class'] = df['class']
            json_df['boxes'] = bounding_boxes
            
            # json_df['nutrition'] = json_df['name'].apply(lambda x: nutrition_dict.get(x, {}))
            
            # Convert the DataFrame to a list of dictionaries
            json_response = json_df.to_dict(orient='records')
            
            # Return the JSON response as a string
            return json.dumps(json_response, indent=2)
        else:
            return json.dumps({"error": "No data available in xyxy[0]"})
    ...
    def pandas_to_json(self):
        ...
        if new.xyxy and not new.xyxy[0].empty:
            ...
            json_df['name'] = df['name']
            json_df['class'] = df['class']
            json_df['confidence'] = df['confidence']
